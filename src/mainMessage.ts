...
let createEmbed = (bigText: string, smallText: string, description: string) => {
...
let createMessage = (bigText: string, smallText: string, description: string, channel: Discord.TextChannel) => {
...
export let editMessage = (bigText: string, smallText: string, description: string, message: Discord.Message) => {
	return new Promise<Discord.Message>((accept, reject) => {
		message.edit(createEmbed(bigText, smallText, description)).then(message => accept(message)).catch(reason => reject(reason));
	});
}

export let updateMessage = (bigText: string, smallText: string, description: string, channel: Discord.TextChannel, message: Discord.Message | undefined = undefined) => {
	return new Promise<Discord.Message>((accept, reject) => {
		if (message) {
